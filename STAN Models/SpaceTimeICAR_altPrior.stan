...
model {
  ...
  for (i in 1:k) {
    target += -(0.5 * sqrt(tau_ICAR_FS)) * dot_self((FS[s_node1,i])' - (FS[s_node2,i])');
  ...
  for (i in 1:k) {
    target += -(0.5 * sqrt(tau_ICAR_L)) * dot_self((L[t_node1,i])' - (L[t_node2,i])');
    // soft sum-to-zero constraint on phi)
    sum(L[,i]) ~ normal(0, 0.001 * m);  // equivalent to mean(phi) ~ normal(0,0.001)
  }
