// Same as SV_spatio-temporal_STAN but with multiplicative gamma shrinkage prior
...
parameters {
  ...
  matrix<lower = 0>[m,k] p; //hyperprior for factor scores (phi in Bhattacharya paper)
  real<lower = 0> delta1;
...
model {
  // added
  for (i in 1:k) L[,i] ~ normal(0, inv(sqrt((p[,i] * tau[i])))); //inv returns 1/x for each element
  ...
  //for (i in 1:k) {
   // target += -0.5 * dot_self((L[t_node1,i])' - (L[t_node2,i])');
  //}
