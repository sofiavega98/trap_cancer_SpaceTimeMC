## This script cleans SEER leukemia incidence data 
...
# Load hispanic and population data from census ages 0-9
...
# Recode data
SEER_1 = SEER %>%
  mutate(cancer_type = recode(`Site recode ICD-O-3/WHO 2008 - Leukemia`, "0" = "Leukemia"),
...
# Load data
CCR_SML <- read_csv("~/cl_quasi_exp/data/CCR_SML.csv")
...
# Add 0s
CA_panel[is.na(CA_panel)] = 0

CA_data <- CA_panel %>% pivot_longer(!CNTYFIPS, names_to = "year", values_to = "count")
CA_data$year <- as.integer(CA_data$year)

## fix fips codes
for(i in 1:nrow(CA_data)){
  if(floor(log10(as.integer(CA_data$CNTYFIPS[i]))) + 1 == 1) {
    CA_data$CNTYFIPS[i] <- paste0("0600",CA_data$CNTYFIPS[i])
  } else if(floor(log10(as.integer(CA_data$CNTYFIPS[i]))) + 1 == 2) {
    CA_data$CNTYFIPS[i] <- paste0("060",CA_data$CNTYFIPS[i])
  } else{
    CA_data$CNTYFIPS[i] <- paste0("06",CA_data$CNTYFIPS[i])
  }
}

## add population and Hispanic rate
## dont have census data from 1988 to 1989 so copy 1990
CA_data_full <- left_join(CA_data,county_hisp_pop, by = c("CNTYFIPS" = "fips", "year" = "year"))

CA_data_full <- CA_data_full %>%
  fill(c(pct_hispanic_pop, total_pop), .direction = "up") 

## specify the treated counties (years greater than or equal to the treated year)
#California
treated1 <- CA_data_full[which(startsWith(CA_data_full$CNTYFIPS,"06")),]
C <- rep(0,dim(treated1)[1])
C[which(treated1$year>=trt_year)] <- 1
treated1 <- cbind(treated1,C)

###############################
## Combine the two data sets ##
###############################

# Subset SEER to start at 1988
data_full <- SEER_final
data_full <- data_full %>% filter(!YEAR_DX <= 1987)
...
# Remove CA from SEER Data up to 2003
data_full <- data_full %>% filter((!(str_starts(FIPS, "06") & YEAR_DX <= 2003)) )

# Specify treated CT Counties
data_full <- data_full %>% mutate(
  # Specify CT as treated after 2003 (should be this but check)
  C = case_when(
    str_starts(FIPS, "09") & YEAR_DX >= trt_year ~ 1,
    TRUE ~ 0),
  # Specify CA as treated after 2003 (should be this but check)
  C = case_when(
    str_starts(FIPS, "06") & YEAR_DX >= 2004 ~ 1, #SEER CA data starts at 2004
    TRUE ~ C)
)
...
data_full_hisp <- data_full_hisp %>% filter(!YEAR_DX > years[length(years)])

data_full_hisp <- data_full_hisp %>%
  fill(c(pct_hispanic_pop, total_pop), .direction = "up") 

# Add CA data to full control
colnames(data_full_hisp) <- c("FIPS","YEAR_DX","CL_CASES","C","pct_hispanic_pop",
                              "POP")
colnames(treated1) <- c("FIPS","YEAR_DX","CL_CASES","pct_hispanic_pop",
                        "POP","C")
# Re-order columns
treated1 <- cbind(treated1$FIPS,as.integer(treated1$YEAR_DX),as.integer(treated1$CL_CASES),as.integer(treated1$C),
                  as.numeric(treated1$pct_hispanic_pop), as.integer(treated1$POP))
colnames(treated1) <- c("FIPS","YEAR_DX","CL_CASES","C","pct_hispanic_pop",
                        "POP")

# Combine data sets
df <- rbind(treated1,data_full_hisp)

df$YEAR_DX <- as.numeric(df$YEAR_DX)
df$CL_CASES <- as.numeric(df$CL_CASES)
df$C <- as.numeric(df$C)
df$pct_hispanic_pop <- as.numeric(df$pct_hispanic_pop)
