...
dir_out_paper="~/cl_quasi_exp/Application/Lymphoma/Manuscript/" # directory output path for results in manuscript
...
load(paste0(dir_out,"processed_res_trt",trt_year,"_SensAnalysis.RData")) #non-smooth
res_nonSmooth <- res
load(paste0(dir_out,"processed_res_trt",trt_year,"_smooth_SensAnalysis.RData")) #smooth
res_Smooth <- res
...
# Load R-hat vectors
load(paste0(dir_out,"rhat_vec_trt",trt_year,"_SensAnalysis.RData"))
rhat_vec_nonSmooth <- rhat_vec
load(paste0(dir_out,"rhat_vec_trt",trt_year,"_smooth_SensAnalysis.RData"))
rhat_vec_Smooth <- rhat_vec

# Combine columns
df <- data.frame(K = as.integer(c(rep(3,5),4)), "Non-Smoothed" = rhat_vec_nonSmooth,"Smoothed" = rhat_vec_Smooth)
colnames(df) <- c("K","Non-Smoothed", "Smoothed")

# Save to combine with Leukemia in a different script
save(df, file = paste0(dir_out,"rhat_df_trt",trt_year,"_SensAnalysis.RData"))
...
save(table, file = paste0(dir_out_paper,"ATT_table",trt_year,"_Smooth_SensAnalysis.RData"))
